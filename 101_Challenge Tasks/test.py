...
print(line_endings)
cv2.imshow("Test",img[49:59,45:53+6])
cv2.waitKey(0)
...
for i in range(0,len(line_beginning)):
    ...
    inter_character_distance = []
    for i in range(1,len(column_slice_beginning)):
        inter_character_distance.append(column_slice_beginning[i]-column_slice_endings[i-1])
    
    print(inter_character_distance)
    
    threshold_separation = mode(inter_character_distance)
    print(threshold_separation)

    for r in range(1,len(column_slice_beginning)):
        d=0
        count=0
        for j in range(0,len(inter_character_distance)):
            if(inter_character_distance[j]<=threshold_separation):
                filename=str(i)+str(j)+'.jpg'
                cv2.imshow("Result",img[x:x2,column_slice_beginning[r-1]:column_slice_endings[r]])
                cv2.waitKey(0)
                cv2.imwrite(filename,img[x:x2,column_slice_beginning[r-1]:column_slice_endings[r]])
